...
namespace API.App_Start
{
    ...
    public static class NinjectWebCommon 
    {
        ...
        private static void RegisterServices(IKernel kernel)
        {
            ...
            kernel.Bind<IContatoAppServico>().To<ContatoAppServico>();
            kernel.Bind<ITelefoneAppServico>().To<TelefoneAppServico>();
            kernel.Bind<IEmailAppServico>().To<EmailAppServico>();

            kernel.Bind(typeof(IServicoBase<>)).To(typeof(ServicoBase<>));
            kernel.Bind<IContatoServico>().To<ContatoServico>();
            kernel.Bind<ITelefoneServico>().To<TelefoneServico>();
            kernel.Bind<IEmailServico>().To<EmailServico>();


            kernel.Bind(typeof(IRepositorioBase<>)).To(typeof(RepositorioBase<>));
            kernel.Bind<IContatoRepositorio>().To<ContatoRepositorio>();
            kernel.Bind<ITelefoneRepositorio>().To<TelefoneRepositorio>();
            kernel.Bind<IEmailRepositorio>().To<EmailRepositorio>();
        }        
    }
}